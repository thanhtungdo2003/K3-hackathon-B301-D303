...
def status() -> dict:
    ...
    return {
        ...
        "tools": [
            {"name": s["function"]["name"], "label": TOOL_LABEL.get(s["function"]["name"], s["function"]["name"])}
...
def chat(
    ...
    if not settings.ai_available:
        return AssistantResponse(reply=UNAVAILABLE, calls=[], source="unavailable", changed=False)
    ...
    def run_tool(name: str, args: dict) -> dict:
        ...
        except Exception as exc:  # noqa: BLE001 — lỗi hệ thống cũng phải quay lại cho LLM biết
            db.rollback()
            return {"error": f"Lỗi khi chạy {name}: {type(exc).__name__}"}

    result = llm.chat_with_tools(history, agent_tools.TOOL_SCHEMAS, run_tool)
    if result is None:
        return AssistantResponse(
            reply="Lúc này không gọi được Groq. Bạn thử lại sau, hoặc làm bằng tay ở các trang bên cạnh.",
            calls=[],
            source="unavailable",
            changed=False,
        )
    ...
    if failure and not result.get("reply"):
        # Gọi hụt giữa đường: vẫn phải báo lại những việc đã kịp làm, không được im.
        result["reply"] = (
    ...
    calls = [
        ToolCallOut(
            tool=c["tool"],
            label=TOOL_LABEL.get(c["tool"], c["tool"]),
            args=c["args"],
            ok="error" not in (c["result"] or {}),
            error=str((c["result"] or {}).get("error") or ""),
            result=c["result"] or {},
        )
        for c in result["calls"]
    ]
    changed = any(c.tool in agent_tools.MUTATING and c.ok for c in calls)
...
def _guarded_reply(reply: str, calls: list[ToolCallOut]) -> str:
    """Hậu kiểm: model từng nói 'đã bắt đầu buổi học' trong khi tool báo lỗi.

    Không tin lời model về việc gì đã xảy ra. Việc thay đổi dữ liệu nào thất bại mà
    không có lần thử lại thành công thì nói thẳng ra, đặt sau câu trả lời của model.
    """
    text = (reply or "").strip() or "Xong."

    succeeded = {c.tool for c in calls if c.ok and c.tool in agent_tools.MUTATING}
    failed = [c for c in calls if not c.ok and c.tool in agent_tools.MUTATING]
    unresolved = [c for c in failed if c.tool not in succeeded]
    if not unresolved:
        return text

    # Gộp theo tool, giữ lỗi đầu tiên của mỗi loại để câu cảnh báo không dài dòng.
    seen: dict[str, str] = {}
    for c in unresolved:
        seen.setdefault(c.label, c.error or "không rõ lý do")
